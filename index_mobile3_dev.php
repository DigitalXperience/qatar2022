...
if($_POST) {
	...
	if ($row = $result->fetch_assoc()) {
		...
		header('Location: index_test_dev.php');
		die();
...
<head>
	...
	<meta charset="UTF-8">
	<title>33 Export Pronostics - Quizz Football</title>
				...
				if (response.status === 'connected') {
					...
					//function getFbUserData(){
						...
						console.log('Name : '+response.name);
							...
							*/
							$.post('_ajax/login_facebook.php', {oauth_provider:'facebook',userData: JSON.stringify(response)}, function(data){ return true; }).done(function() { 
								window.location.href='index_test_dev.php'; 
							});
						});
					//}
			...
			if (response.status === 'connected') {
				...
				FB.api('/me', {fields: 'id,name,email,picture'},
					function (response) {
						...
						document.getElementById('userData').innerHTML = '<p><b>FB ID:</b> '+response.id+'</p><p><b>Name:</b> '+response.first_name+' '+response.last_name+'</p><p><b>Email:</b> '+response.email+'</p><p><b>Gender:</b> '+response.gender+'</p><p><b>Locale:</b> '+response.locale+'</p><p><b>Picture:</b> <img src="'+response.picture.data.url+'"/></p><p><b>FB Profile:</b> <a target="_blank" href="'+response.link+'">click to view profile</a></p>';
						*/
						$.post('_ajax/continue_with_facebook_dev.php', {oauth_provider:'facebook',userData: JSON.stringify(response)}, function(data){ return true; }).done(function() { 
							window.location.href='index_test_dev.php'; 
						});
					});
			}
	...
	<div class="login-register-wrap-home">
		<div class="container">
			<div class="content">	
				...
				<form action="https://www.33export-foot.com/quizz/index_mobile3.php" id="form_login" style="margin-top:15px" method="post">
